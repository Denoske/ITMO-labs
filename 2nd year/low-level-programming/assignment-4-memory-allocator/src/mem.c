...
static struct region alloc_region(void const *addr, size_t query) {
    ...
    void *mapped_pages = map_pages(addr, region_size, MAP_FIXED_NOREPLACE);

    if (mapped_pages != MAP_FAILED) {
        final_address = mapped_pages;
    } else {
        mapped_pages = map_pages(addr, region_size, 0);
        if (mapped_pages != MAP_FAILED) {
            final_address = mapped_pages;
        }
    }
    ...
    if(final_address){
      block_init(final_address, (block_size) {.bytes = region_size}, NULL);
    }
    
    return (struct region) {.addr = final_address, .size = region_size, .extends = (final_address ==addr)};
}
...
static bool split_if_too_big( struct block_header* block, size_t query ) {
  ...
  block_init((block->contents) + query, (block_size) {.bytes=block->capacity.bytes - query}, block->next);
...
static struct block_header* memalloc( size_t query, struct block_header* heap_start) {
  struct block_search_result bsr = try_memalloc_existing(size_max(query, BLOCK_MIN_CAPACITY), heap_start);
  switch(bsr.type){
    case BSR_CORRUPTED:
      return NULL;
    ...
    case BSR_REACHED_END_NOT_FOUND:
      return try_memalloc_existing(size_max(query, BLOCK_MIN_CAPACITY), grow_heap(bsr.block, size_max(query, BLOCK_MIN_CAPACITY))).block;
  }
  /*  ??? */
