...
static char check_frame(struct sk_buff *skb, unsigned char data_shift) {
	...
	if (IPPROTO_UDP == ip->protocol) {
        ...
        if (htons(udp->dest) == target_port)
        {
            ...
            end_char += sprintf(summ_str + end_char, "source: %d.%d.%d.%d:%d, destination: %d.%d.%d.%d:%d\n",
                                ntohl(ip->saddr) >> 24, (ntohl(ip->saddr) >> 16) & 0x00FF,
                                (ntohl(ip->saddr) >> 8) & 0x0000FF, (ntohl(ip->saddr)) & 0x000000FF, htons(udp->source),
                                ntohl(ip->daddr) >> 24, (ntohl(ip->daddr) >> 16) & 0x00FF,
                                (ntohl(ip->daddr) >> 8) & 0x0000FF, (ntohl(ip->daddr)) & 0x000000FF, htons(udp->dest));    
            summ_str[end_char] = '\0';
        }
        return 1;
